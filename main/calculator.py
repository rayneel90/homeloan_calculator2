...
def calculation(prop_price, downpay, roi, tenure, annual_early_repay,
                date_of_purchase, months_to_possession, monthly_rent_earning ,
                annual_maintenance, annual_prop_tax , limit_80c , self_pf_contri,
                int_rebate_limit, rd_rate, premi=False, **kwargs):
    roi = roi/1200
    ...
    dat = [{'date': date_of_purchase, 'pos': 0, 'emi': 0, 'interest_paid': 0,
    ...
    #     df = df.append(df_fy)
    #
    # df = df[['date', 'pos', 'emi', 'principle_repaid', 'interest_paid',
    #          'extra_repaid',  'maintenance', 'prop_tax', 'rent', 'tax_save_princi',
    #          'tax_save_interest', 'unclaimed_int_amnt']]
    #
    ...
    # df['rd_return'] = df.apply(lambda x: x['net_outflow']*(1+rd_rate/4)**(
